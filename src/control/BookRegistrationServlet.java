...
public class BookRegistrationServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		req.getRequestDispatcher("admin_register_book.jsp").forward(req, resp);;
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Book book = new Book(isbn, title, author, date, price, categoryId, true);
		...
		if (result) {
			...
			session.setAttribute("message", "登録に失敗しました　ISBN番号は13桁以内で入力してください");
		} else {
			session.setAttribute("message", "登録に失敗しました　既存のISBN番号の可能性があります");
		}
